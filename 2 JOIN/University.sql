...
--Запросы 
--1. Вывести все возможные пары преподавателей и групп. (Randomnoe  raspisanie universiteta)


SELECT Groups.[Name],Groups.[Year],Lectures.LectureRoom,CONCAT(Teachers.[Name],' ' + Teachers.Surname) AS 'Teacher name',Subjects.[Name] AS 'Subject'
FROM Groups
...
FROM Faculties ,(SELECT *
                FROM Departments) AS sub
WHERE Faculties.Id = sub.FacultyId AND sub.Financing> Faculties.Financing
...
SELECT DISTINCT Faculties.[Name]
FROM Faculties 
WHERE Faculties.Financing < ANY(SELECT Departments.Financing
                                FROM Departments
...
--4. Вывести имена и фамилии преподавателей, которые читают лекции у группы “P107”. (FSDM_3911_ru)
...
WHERE Groups.[Name] = 'FSDM_3911_ru'
...
--7. Вывести названия дисциплин, которые читает преподаватель “Samantha Adams”. (Modesty Lacas)

SELECT Subjects.[Name] AS 'Subject name'
FROM Subjects
...
--8. Вывести названия кафедр, на которых читается дисциплина “Database Theory”.(Computer Networks (5th Edition)) 
...
JOIN Lectures ON GroupsLectures.LectureId = Lectures.Id
JOIN Subjects ON Lectures.SubjectId = Subjects.Id
WHERE Subjects.[Name] LIKE 'Computer Networks (5th Edition)'


--9. Вывести названия групп, которые относятся к факультету “Computer Science”. (Software Development)
...
--которые они читают (названия дисциплин и групп), причем отобрать 
--только те лекции, которые читаются в аудитории “B103”. (FSDM_6911_az)

SELECT CONCAT(Teachers.[Name],' ' + Teachers.Surname),Subjects.[Name],Groups.[Name]
FROM Teachers
JOIN Lectures ON Lectures.TeacherId = Teachers.Id
JOIN Subjects ON Lectures.SubjectId = Subjects.Id
JOIN GroupsLectures ON GroupsLectures.LectureId = Lectures.Id
JOIN Groups ON GroupsLectures.GroupId = Groups.Id
WHERE Groups.[Name] LIKE 'FSDM_6911_ru'
